...
def configure_app(app, config_path=None):
    ...
    if cfg is not None:
        ...
        if isinstance(cfg, (str, Path)):
            ...
            sources["config_path"] = getqualname(cfg)
    ...
    envvars = config_from_env("GAMENIGHT_", ignore=frozenset(["GAMENIGHT_SETTINGS"]))
...
def register_blueprints(app):
    bps = [
        bp for bp in blueprints.__dict__.values() if isinstance(bp, GamenightBlueprint)
    ]
    for bp in bps:
